        ...
        if (settings.getSettings().isSearchUpdates()) {
            if (settings.getSettings().isUseDefaultRepo()) {
                ...
                if ("dev".equals(System.getProperty("correo.mode"))) {
                    defaultRepo = Path.of("target", "default-repo.json").toAbsolutePath().toUri().toString();
                    LOGGER.warn("DEV MODE: Reading plugins repo from {}", defaultRepo);
                } else if (defaultRepo.contains("{version}")) {
